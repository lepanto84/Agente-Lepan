...
registrar_evento("🤖 AGENTE LEPAN [MODO MICRO-INVERSIÓN] INICIADO.")
...
while True:
    try:
        ...
        en_posicion = saldo_cripto > 0.0001 
        ...
        # Ejecución con Gestión de Capital
        if cruce_alcista and rsi < 70 and precio > df['SMA_100'].iloc[-2] and not en_posicion and saldo_usdt > 10:
            # Calculamos la cantidad a comprar basándonos en el saldo total
            cantidad_a_comprar = (saldo_usdt * 0.98) / precio
            orden = exchange.create_market_buy_order(SIMBOLO, cantidad_a_comprar)
            registrar_evento(f"🚀 COMPRA TOTAL: {cantidad_a_comprar:.6f} BTC usando {saldo_usdt:.2f} USDT")

        elif cruce_bajista and en_posicion:
            orden = exchange.create_market_sell_order(SIMBOLO, saldo_cripto)
            registrar_evento(f"🚨 VENTA TOTAL: Se liquidó todo el {MONEDA} a {orden['average']} USDT")

    except Exception as e:
        registrar_evento(f"❌ ERROR DE EJECUCIÓN: {e}")
